...
def activity_overview(tag,title,desc,build,services,kicker):
    ...
    # The full lab description lives in the Learner Guide; the slide shows a concise
    # lead-in — drop the trailing project-note and truncate at a sentence boundary.
    d=str(desc).split("BUILDING BLOCK")[0].strip()
    if len(d)>400:
        cut=d[:400]; dot=max(cut.rfind(". "),cut.rfind("; "))
        d=((cut[:dot+1] if dot>220 else cut.rstrip())+" …")
    dL=len(d); dsize = 21 if dL<=200 else (18 if dL<=300 else 16)
    txt(s,Inches(0.85),Inches(2.55),Inches(11.7),Inches(1.6),[[(d,dsize,_ink(),False)]])
    rect(s,Inches(0.85),Inches(4.3),Inches(11.7),Inches(2.0),_panel())
    txt(s,Inches(1.1),Inches(4.45),Inches(11),Inches(0.4),[[("You'll build",14,_acc(BLUE),True)]])
    bL=len(str(build)); bsize = 18 if bL<=150 else (15 if bL<=250 else 13)
    txt(s,Inches(1.1),Inches(4.85),Inches(11.2),Inches(0.9),[[(build,bsize,_ink(),True)]])
    txt(s,Inches(1.1),Inches(5.78),Inches(11.2),Inches(0.5),[[("Tools:  ",13,_grey(),True),(services,13,_grey(),False)]]); footer(s); return s
def step_slide(kicker,act_title,n,total,text,cmd=""):
    ...
    if cmd and not cmd.lstrip().startswith("#"):
        # Auto-fit the prompt box so long prompts never clip past the box edge.
        clen=len(cmd)
        csize = 13 if clen<=300 else 12
        ch = 0.95 if clen<=300 else (1.28 if clen<=430 else 1.6)
        if THEME["dark"]:
            # slightly lighter fill + faint border so the box reads against the dark bg
            rect(s,Inches(2.55),Inches(4.15),Inches(10.1),Inches(ch),RGBColor(0x10,0x14,0x1E),line=DK_FAINT)
        else:
            rect(s,Inches(2.55),Inches(4.15),Inches(10.1),Inches(ch),RGBColor(0x0B,0x12,0x20))
        txt(s,Inches(2.8),Inches(4.28),Inches(9.7),Inches(ch-0.26),[[("$ "+cmd,csize,RGBColor(0x9C,0xDC,0xFE),False)]],anchor=MSO_ANCHOR.MIDDLE)
    footer(s); return s
...
_right=[(f"Day {_d2} — {C.DAY_THEMES[_d2]}",0)]+[_topic_line(_TB[n]) for n in _BY_DAY.get(_d2,[]) if n in _TB]+[
 ("Daily timing",0),("9:00am–6:00pm · 1-hour lunch · tea breaks within training time",1)]
